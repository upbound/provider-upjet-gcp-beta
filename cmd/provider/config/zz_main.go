...
func main() {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-gcp-beta-config",
		Cache: cache.Options{
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated_config(mgr, optCluster), "Cannot setup GCP controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated_config(mgr, optNamespaced), "Cannot setup GCP controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup_config(mgr, optCluster), "Cannot setup GCP controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup_config(mgr, optNamespaced), "Cannot setup GCP controllers")
	}
