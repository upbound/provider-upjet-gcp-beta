...
func main() {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-gcp-beta-container",
		Cache: cache.Options{
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated_container(mgr, optCluster), "Cannot setup GCP controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated_container(mgr, optNamespaced), "Cannot setup GCP controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup_container(mgr, optCluster), "Cannot setup GCP controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup_container(mgr, optNamespaced), "Cannot setup GCP controllers")
	}
