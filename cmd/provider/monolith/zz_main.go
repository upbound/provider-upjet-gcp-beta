...
func main() {
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated_monolith(mgr, optCluster), "Cannot setup GCP controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated_monolith(mgr, optNamespaced), "Cannot setup GCP controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup_monolith(mgr, optCluster), "Cannot setup GCP controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup_monolith(mgr, optNamespaced), "Cannot setup GCP controllers")
	}
