...
## Configure the provider-family-gcp-beta

The GCP Beta provider-family requires credentials for authentication to Google Cloud Platform.
...
$ PROVIDER_GCP=<YOUR_PROVIDER_GCP_NAME>                          # e.g.) provider-gcp-beta-storage
...
Now that you have configured `provider-gcp-beta-storage` with Workload Identity supported.
...
Now you have configured `provider-gcp-beta-storage` with Impersonated Service Account support.
...
`provider-gcp-beta-storage` can be installed with either
...
Now that you have configured `provider-gcp-beta-storage` with Access Tokens supported,
